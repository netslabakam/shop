...
abstract class autoCategoryActions extends sfActions
{
  ...
  public function executeNew(sfWebRequest $request)
  {
    ...
    $this->shop_category = $this->form->getObject();
  }
  ...
  {
    $this->shop_category = $this->getRoute()->getObject();
    $this->form = $this->configuration->getForm($this->shop_category);
  }
  ...
  protected function processForm(sfWebRequest $request, sfForm $form)
  {
    ...
    if ($form->isValid())
    {
      ...
      $this->dispatcher->notify(new sfEvent($this, 'admin.save_object', array('object' => $shop_category)));

      if ($request->hasParameter('_save_and_add'))
      {
        $this->getUser()->setFlash('notice', $notice.' You can add another one below.');

        $this->redirect('@shop_category_new');
      }
      else
      {
        $this->getUser()->setFlash('notice', $notice);

        $this->redirect(array('sf_route' => 'shop_category_edit', 'sf_subject' => $shop_category));
      }
    }
    else
    {
  ...
  {
    $pager = $this->configuration->getPager('ShopCategory');
  ...
  protected function buildQuery()
  {
    ...
    $query = Doctrine::getTable('ShopCategory')
      ->createQuery('a');

    if ($tableMethod)
    {
      $query = Doctrine::getTable('ShopCategory')->$tableMethod($query);
    }
  ...
  {
    return Doctrine::getTable('ShopCategory')->hasColumn($column);
  }
}
