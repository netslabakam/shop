...
$t->is(javascript_include_tag('xmlhr'),
  '<script type="text/javascript" src="/jscript/xmlhr.jscript"></script>'."\n", 
...
$t->is(javascript_include_tag('common.javascript', '/elsewhere/cools'),
  '<script type="text/javascript" src="/jscript/common.javascript"></script>'."\n".
  '<script type="text/javascript" src="/elsewhere/cools.jscript"></script>'."\n",
...
$t->is(javascript_include_tag('xmlhr', array('absolute' => true)),
  '<script type="text/javascript" src="http://localhost/jscript/xmlhr.jscript"></script>'."\n", 
...
$t->is(javascript_include_tag('xmlhr', array('defer' => 'defer')),
  '<script type="text/javascript" src="/jscript/xmlhr.jscript" defer="defer"></script>'."\n", 
  'javascript_include_tag() can take additional html options like defer');
$t->is(javascript_include_tag('xmlhr', array('condition' => 'IE 6')),
  '<!--[if IE 6]><script type="text/javascript" src="/jscript/xmlhr.jscript"></script><![endif]-->'."\n",
  'javascript_include_tag() can take a condition option');

// javascript_path()
$t->diag('javascript_path()');
$t->is(javascript_path('xmlhr'), '/jscript/xmlhr.jscript', 'javascript_path() decorates a relative filename with jscript dir name and extension');
$t->is(javascript_path('/xmlhr'), '/xmlhr.jscript', 'javascript_path() does not decorate absolute file names with jscript dir name');
$t->is(javascript_path('xmlhr.foo'), '/jscript/xmlhr.foo', 'javascript_path() does not decorate file names with extension with .jscript');
$t->is(javascript_path('xmlhr.foo', true), 'http://localhost/jscript/xmlhr.foo', 'javascript_path() accepts a second parameter to output an absolute resource path');
...
$t->is(get_javascripts(),
  '<script type="text/javascript" src="/jscript/xmlhr.jscript"></script>'."\n".'<script type="text/javascript" src="/jscript/xmlhr2.jscript"></script>'."\n", 
...
$t->is(_dynamic_path('module/action', 'jscript'), 'module/action?sf_format=jscript', '_dynamic_path() converts an internal URI to a URL');
$t->is(_dynamic_path('module/action?key=value', 'jscript'), 'module/action?key=value&sf_format=jscript', '_dynamic_path() converts an internal URI to a URL');
$t->is(_dynamic_path('module/action', 'jscript', true), '/module/action?sf_format=jscript', '_dynamic_path() converts an internal URI to a URL');

// dynamic_javascript_include_tag()
$t->diag('dynamic_javascript_include_tag()');
$t->is(dynamic_javascript_include_tag('module/action'), '<script type="text/javascript" src="module/action?sf_format=jscript"></script>'."\n", 'dynamic_javascript_include_tag() returns a tag relative to the given action');
$t->is(dynamic_javascript_include_tag('module/action', true), '<script type="text/javascript" src="/module/action?sf_format=jscript"></script>'."\n", 'dynamic_javascript_include_tag() takes an absolute boolean as its second argument');
$t->is(dynamic_javascript_include_tag('module/action', true, array('class' => 'foo')), '<script type="text/javascript" src="/module/action?sf_format=jscript" class="foo"></script>'."\n", 'dynamic_javascript_include_tag() takes an array of HTML attributes as its third argument');
...
$t->is(get_javascripts(),
  '<script type="text/javascript" src="module/action?sf_format=jscript"></script>'."\n",
...
class MyForm extends sfForm
{
  ...
  {
    return array('/path/to/a/foo.jscript', '/path/to/a/bar.jscript');
  }
}
...
$form = new MyForm();
$output = <<<EOF
<script type="text/javascript" src="/path/to/a/foo.jscript"></script>
<script type="text/javascript" src="/path/to/a/bar.jscript"></script>
...
$t->is_deeply($response->getJavaScripts(), array('/path/to/a/foo.jscript' => array(), '/path/to/a/bar.jscript' => array()), 'use_javascripts_for_form() adds javascripts to the response');

// custom web paths
$t->diag('Custom asset path handling');

sfConfig::set('sf_web_js_dir_name', 'static/jscript');
$t->is(javascript_path('xmlhr'), '/static/jscript/xmlhr.jscript', 'javascript_path() decorates a relative filename with jscript dir name and extension with custom jscript dir');
$t->is(javascript_include_tag('xmlhr'),
  '<script type="text/javascript" src="/static/jscript/xmlhr.jscript"></script>'."\n", 
  'javascript_include_tag() takes a javascript name as its first argument');
