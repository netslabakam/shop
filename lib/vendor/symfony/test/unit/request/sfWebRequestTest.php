...
$request->setParameter('sf_format', 'jscript');
$t->is($request->getRequestFormat(), 'jscript', '->getRequestFormat() returns the request format');
...
$request->setFormat('jscript', 'application/x-javascript');
$t->is($request->getFormat('application/x-javascript'), 'jscript', '->getFormat() returns the format for the given mime type');
$request->setFormat('jscript', array('application/x-javascript', 'text/jscript'));
$t->is($request->getFormat('text/jscript'), 'jscript', '->setFormat() can take an array of mime types');
$t->is($request->getFormat('foo/bar'), null, '->getFormat() returns null if the mime type does not exist');

// ->getMimeType()
$t->diag('->getMimeType()');
$t->is($request->getMimeType('jscript'), 'application/x-javascript', '->getMimeType() returns the first mime type for the given format');
