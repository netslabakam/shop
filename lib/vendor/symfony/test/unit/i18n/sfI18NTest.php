...
$dispatcher->notify(new sfEvent(null, 'user.change_culture', array('culture' => 'ru')));
$t->is($i18n->getCulture(), 'ru', '->initialize() connects to the user.change_culture event');

// ->getCulture() ->setCulture()
$t->diag('->getCulture() ->setCulture()');
$i18n = new sfI18N($configuration, $cache);
$t->is($i18n->getCulture(), 'ru', '->getCulture() returns the current culture');
$i18n->setCulture('ru');
$t->is($i18n->getCulture(), 'ru', '->setCulture() sets the current culture');

// ->__()
$t->diag('->__()');
sfConfig::set('sf_charset', 'UTF-8');
$i18n = new sfI18N($configuration, $cache, array('culture' => 'ru'));
...
$t->diag('->getCountry()');
$i18n = new sfI18N($configuration, $cache, array('culture' => 'ru'));
$t->is($i18n->getCountry('RU'), 'Russia', '->getCountry() returns the name of a country for the current culture');
$t->is($i18n->getCountry('RU', 'ru'), 'Russia', '->getCountry() takes an optional culture as its second argument');

// ->getNativeName()
$t->diag('->getNativeName()');
$i18n = new sfI18N($configuration, $cache, array('culture' => 'ru'));
$t->is($i18n->getNativeName('ru'), 'russia', '->getNativeName() returns the name of a culture');
...
$t->is($i18n->getTimestampForCulture('10/15/2005', 'ru_RU'), mktime(0, 0, 0, '10', '15', '2005'), '->getTimestampForCulture() can take a culture as its second argument');
$t->is($i18n->getTimestampForCulture('10/15/2005 3:33 pm', 'ru_RU'), mktime(15, 33, 0, '10', '15', '2005'), '->getTimestampForCulture() can take a culture as its second argument');
...
$t->is($i18n->getDateForCulture('10/15/2005', 'ru_RU'), array('15', '10', '2005'), '->getDateForCulture() can take a culture as its second argument');
...
$t->is($i18n->getTimeForCulture('3:33 pm', 'ru_RU'), array('15', '33'), '->getTimeForCulture() can take a culture as its second argument');
